...
const UnStake: React.FC<UnStakeProps> = ({
    ...
    const onSelectPercent = (percent: number = 1) => {
        const amount = xStepTokenBalance * percent;
    ...
    const onSubmit = async () => {
        await unStake(Number(unStakeAmount));
        getTokenBalance();
    };
        ...
        <Space direction="vertical" className="bg-[#141414] w-[450px] p-[20px] rounded-b-lg mt-[-16px]">
            ...
            <div className="flex justify-between items-center bg-black rounded-lg p-[12px]">
                <div className="flex justify-center items-center">
                    <Image alt="xstep" width={28} height={28} src="/xstep.svg" className="rounded-full" />
            ...
            <div className="flex justify-between items-center bg-black rounded-lg p-[12px]">
                <div className="flex justify-center items-center">
                    <Image alt="step" width={28} height={28} src="/step.png" className="rounded-full" />
            ...
            <Button
                ...
                type="primary"
                onClick={onSubmit}
                disabled={!connected || !unStakeAmount || xStepTokenBalance < Number(unStakeAmount)}
            >
                {!connected
                    ? 'Connect Wallet'
                    : !unStakeAmount
                    ? 'Enter an amount'
                    : xStepTokenBalance < Number(unStakeAmount)
                    ? 'Insufficient xSTEP balance'
                    : 'UnStake'}
            </Button>
        </Space>
    );
